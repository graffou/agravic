...
volatile uint32_t *spi_data = (volatile uint32_t *) CONV_ADDR(SPI_MST_REGS)+4;//0x40007f44;
volatile uint32_t *spi_conf_slv = (volatile uint32_t *) CONV_ADDR(SPI_SLV_REGS);//0x40007f00;
volatile uint32_t *spi_data_slv = (volatile uint32_t *) CONV_ADDR(SPI_SLV_REGS)+4;//0x40007f04;
// second uart is included in testbench
// configure it with a -0x400 (or 0x100 in 4B words) offset
// this verification hack is found in tb.h
